pipeline {
    ...
    stages {
        stage('Initialize Git POLL SCM Feature') {
            steps {
                ...
                script {
                    properties([[$class: 'GithubProjectProperty', displayName: '', projectUrlStr: 'https://github.com/Anilkumar4599/CICD-deployment-using-Ansible-CM-tool.git'], pipelineTriggers([pollSCM('* * * * *')])])
                }  
            }
        }
	...
	stage('Git project checkout') {
        steps {
               git branch: 'master', url: 'https://github.com/Anilkumar4599/CICD-deployment-using-Ansible-CM-tool.git'  
          }
        }
